(function () {
  ...
  const methodSection = document.getElementById("process");
  const editorialReveals = Array.from(document.querySelectorAll(".editorial-reveal"));
  ...
  let editorialLightVisible = false;
  let editorialDarkVisible = false;
  let storyVisible = false;
  let storyLightVisible = false;
  ...
  function updateScrollTarget() {
    ...
    editorialDarkVisible = Boolean(
      serviceCtaRect
      && serviceCtaRect.top < height * 0.7
  ...
  function updateActiveNavigation() {
    if (!activeNavigationEntries.length) return;
    const marker = window.scrollY + Math.max(120, window.innerHeight * 0.36);
    let current = activeNavigationEntries[0];

    activeNavigationEntries.forEach((entry) => {
      if (entry.target.offsetTop <= marker) current = entry;
    });

    orbitNavLinks.forEach((link) => {
  ...
  function updateDocumentMotion() {
    ...
    const lightSceneActive = (sceneVisible && smoothSceneEntry > 0.42 && !orbitVisible)
      || orbitStillLight
      || (editorialLightVisible && !editorialDarkVisible)
      || storyLightVisible;
    ...
    if (editorialLightVisible) {
      editorialMedia.forEach((media) => {
        const rect = media.getBoundingClientRect();
        const travel = clamp((height - rect.top) / (height + rect.height), 0, 1);
        media.style.setProperty("--media-y", `${mix(-8, -3, travel).toFixed(3)}%`);
      });
    }

    if (storyVisible && !reducedMotion.matches) {
      storyMedia.forEach((media) => {
        const rect = media.getBoundingClientRect();
        const travel = clamp((height - rect.top) / (height + rect.height), 0, 1);
        media.style.setProperty("--story-y", `${mix(-7, -2, travel).toFixed(3)}%`);
      });
    }
  }

  function setupEditorialReveals() {
    if (!editorialReveals.length) return;
    if (reducedMotion.matches || !("IntersectionObserver" in window)) {
      editorialReveals.forEach((element) => element.classList.add("is-visible"));
      return;
    }

    body.classList.add("has-editorial-reveal");
    ...
    editorialReveals.forEach((element) => observer.observe(element));
  }

  function setupIcons() {
    document.querySelectorAll(".service-disclosure__trigger > i").forEach((icon) => {
    ...
    const ctaIcon = document.querySelector(".service-cta__action > a > i");
    if (ctaIcon) ctaIcon.dataset.lucide = "arrow-up-right";

    document.querySelectorAll(".service-editorial__link > i").forEach((icon) => {
    ...
    faqItems.forEach((item) => {
      ...
      button.addEventListener("click", () => {
        ...
        faqItems.forEach((candidate) => {
          const candidateButton = candidate.querySelector(".faq-question");
          const candidateAnswer = candidate.querySelector(".faq-answer");
          const shouldOpen = candidate === item && willOpen;
  ...
  function setupServiceDisclosures() {
    serviceDisclosureGroups.forEach((group) => {
      const items = Array.from(group.querySelectorAll(".service-disclosure"));

      items.forEach((item) => {
        const button = item.querySelector(".service-disclosure__trigger");
        const answer = item.querySelector(".service-disclosure__answer");
        if (!button || !answer) return;
        ...
        button.addEventListener("click", () => {
          ...
          items.forEach((candidate) => {
            const candidateButton = candidate.querySelector(".service-disclosure__trigger");
            const candidateAnswer = candidate.querySelector(".service-disclosure__answer");
            const shouldOpen = candidate === item && willOpen;
            candidate.classList.toggle("is-open", shouldOpen);
            if (candidateButton) candidateButton.setAttribute("aria-expanded", String(shouldOpen));
            if (candidateAnswer) candidateAnswer.setAttribute("aria-hidden", String(!shouldOpen));
